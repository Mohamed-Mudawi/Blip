...
export default function SocialMediaPoster() {
    ...
    const handlePost = async () => {
        ...
        try {
            // If there are files with File objects, upload them first to our uploads API
            let uploadedUrls: string[] = [];
            const filesToUpload = mediaFiles.map((m) => m.file).filter(Boolean) as File[];
            if (filesToUpload.length > 0) {
                ...
                uploadedUrls = mediaFiles.map((m) => m.url);
            }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-blue-900 p-6 md:p-12 text-white">
            <div className="max-w-5xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-blue-200/80">
                        Post to multiple platforms at once — fast, simple, and start-up ready
                    </p>
                </div>

                {/* Error Banner */}
                {error && (
                    <div className="mb-6 bg-red-900/80 border border-red-700/40 rounded-lg p-4 flex items-center gap-3">
                ...
                {/* Success Banner */}
                {showSuccess && (
                    <div className="mb-6 bg-green-800/80 border border-green-600/30 rounded-lg p-4 flex items-center gap-3">
                ...
                {/* Main Card */}
                <div className="bg-black/60 border border-white/6 rounded-2xl shadow-lg p-6 mb-6 backdrop-blur-sm">
                            ...
                            {platforms.map((platform) => (
                                <button
                                    ...
                                    className={`relative p-4 rounded-xl transition-all focus:outline-none ${selectedPlatforms[
                                            ...
                                            ? `bg-gradient-to-br from-blue-600 to-black text-white shadow-lg scale-105`
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex gap-2 mb-4">
                            <label className="cursor-pointer bg-white/5 hover:bg-white/10 text-blue-200 px-4 py-2 rounded-lg flex items-center gap-2 transition-colors focus:outline-none">
                                <ImageIcon size={20} />
                                ...
                                {mediaFiles.map((file) => (
                                    <div key={file.id} className="relative group">
                                        ...
                                        <button
                                            ...
                                            className="absolute top-2 right-2 bg-red-600/80 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity focus:outline-none"
                                        >
                                            <X size={16} />
                                        </button>
                                    </div>
                    ...
                    <button
                        ...
                        onClick={handlePost}
                        disabled={isPosting}
                        className="w-full bg-gradient-to-r from-blue-600 to-black hover:from-blue-700 hover:to-black text-white font-semibold py-3 rounded-xl transition-all shadow-lg hover:shadow-xl focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                        {isPosting ? (
                            <>
